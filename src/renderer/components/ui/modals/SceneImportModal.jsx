...
import { parseScenes } from '../../../utils/parseScenes.js'

/**
 * SceneImportModal - 场景导入悬浮面板
 *
 * 流程：用户选择 .txt 文件或粘贴文本 → 按"场景1/场景一"解析 → 展示结果 → 选择生成视频/图片节点
 */
export function SceneImportModal({ open, onClose, onCreateNodes }) {
  ...
  const handleFileSelect = useCallback((selectedFile) => {
    ...
    reader.onload = (e) => {
      ...
      setParseResult(parseScenes(text))
    }
  ...
  const handleParsePaste = useCallback(() => {
    ...
    setFile({ name: '粘贴文本' })
    setParseResult(parseScenes(pasteText))
  }, [pasteText])
      ...
      >
        ...
        >
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-sm font-semibold text-[var(--text-primary)]">场景导入</h3>
              <p className="text-[10px] text-[var(--text-muted)] mt-0.5">
                导入 TXT 文件或粘贴文本，按「场景1」「场景2」解析
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto min-h-0 px-5 py-4">
          {!parseResult ? (
            <div className="flex flex-col gap-4">
              ...
              <div
                className={`border-2 border-dashed rounded-xl p-6 text-center transition-all duration-200 cursor-pointer ${
                  dragOver
                    ...
                    : 'border-[var(--border-color)] hover:border-blue-400/50 hover:bg-[var(--bg-hover)]'
                }`}
                onDrop={handleDrop}
              ...
              {/* 粘贴区 */}
              <textarea
                className="w-full h-32 bg-[var(--bg-secondary)] text-xs outline-none resize-none custom-scrollbar text-[var(--text-primary)] placeholder-[var(--text-muted)] leading-relaxed p-3 rounded-xl border border-[var(--border-color)] focus:border-blue-400/50 transition-colors"
                placeholder={
                  '粘贴包含「场景1」「场景2」等标记的内容...\n\n例如：\n场景1\n深夜的码头，月光洒在水面上\n\n场景2\n清晨的办公室，阳光透过窗帘'
                }
                value={pasteText}
