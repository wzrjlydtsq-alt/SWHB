...
import { parseCharacters } from '../../../utils/parseCharacters.ts'

/**
 * CharacterImportModal - 角色导入悬浮面板
 *
 * 流程：用户选择 .txt 文件或粘贴文本 → 按"角色1/角色一"解析 → 展示结果 → 选择生成视频/图片节点
 */
export function CharacterImportModal({ open, onClose, onCreateNodes }) {
  ...
  const handleFileSelect = useCallback((selectedFile) => {
    if (!selectedFile || !selectedFile.name.endsWith('.txt')) return
    setFile(selectedFile)
    const reader = new FileReader()
    reader.onload = (e) => {
      const text = e.target.result
      setParseResult(parseCharacters(text))
    }
  ...
  const handleParsePaste = useCallback(() => {
    ...
    setFile({ name: '粘贴文本' })
    setParseResult(parseCharacters(pasteText))
  }, [pasteText])
      ...
      >
        ...
        >
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-sm font-semibold text-[var(--text-primary)]">角色导入</h3>
              <p className="text-[10px] text-[var(--text-muted)] mt-0.5">
                导入 TXT 文件或粘贴文本，按「角色1」「角色2」解析
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto min-h-0 px-5 py-4">
          {!parseResult ? (
            <div className="flex flex-col gap-4">
              ...
              <div
                className={`border-2 border-dashed rounded-xl p-6 text-center transition-all duration-200 cursor-pointer ${
                  dragOver
                    ...
                    : 'border-[var(--border-color)] hover:border-emerald-400/50 hover:bg-[var(--bg-hover)]'
                }`}
                onDrop={handleDrop}
              ...
              {/* 粘贴区 */}
              <textarea
                className="w-full h-32 bg-[var(--bg-secondary)] text-xs outline-none resize-none custom-scrollbar text-[var(--text-primary)] placeholder-[var(--text-muted)] leading-relaxed p-3 rounded-xl border border-[var(--border-color)] focus:border-emerald-400/50 transition-colors"
                placeholder={
                  '粘贴包含「角色1」「角色2」等标记的内容...\n\n例如：\n角色1\n张小明，25岁男性，短发\n\n角色2\n李小花，22岁女性，长发'
                }
                value={pasteText}
