...
const CreateCharacterModal = lazy(() =>
  import('./modals/CreateCharacterModal.tsx').then((m) => ({
    default: m.CreateCharacterModal
  }))
)
const SettingsModal = lazy(() =>
  import('./modals/SettingsModal.tsx').then((m) => ({
    default: m.SettingsModal
  }))
)
const BatchModal = lazy(() =>
  import('./modals/BatchModal.tsx').then((m) => ({
...
export function ModalManager({
  ...
  const modalLoading =
    settingsOpen || batchModalOpen || createCharacterOpen ? (
      <div className="fixed inset-0 z-[200000] flex items-center justify-center bg-black/30">
        <div className="rounded-lg border border-white/10 bg-[var(--bg-panel)] px-4 py-3 text-sm text-[var(--text-secondary)] shadow-xl">
          正在打开...
        </div>
      </div>
    ) : null
