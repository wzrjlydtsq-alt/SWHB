...
import { ChevronRight, Play, Crosshair } from '../../utils/icons.tsx'
...
export const SceneExtractNode = ({
  ...
  return (
    ...
    >
      ...
      {/* 底部工具栏 */}
      <div className="node-footer-controls flex items-center gap-1.5 shrink-0 px-3 py-2 border-t border-[var(--border-color)]">
        {/* 模型选择 */}
        <div className="relative">
          ...
          >
            <span className="truncate max-w-[80px]">
              {currentModel?.modelName || currentModel?.id || '选择模型'}
            </span>
          ...
          {activeDropdown === 'model' && (
            ...
            >
              {chatModels.map((m) => (
                ...
                >
                  <span className="text-xs font-medium truncate pr-2">{m.modelName || m.id}</span>
        ...
        <button
          ...
          className="node-footer-primary ml-auto flex items-center gap-1 px-3 py-1 rounded-lg text-[10px] font-bold transition-all active:scale-95 bg-[var(--primary-color)] text-white hover:opacity-90 shadow-md disabled:opacity-50 disabled:cursor-not-allowed"
          onMouseDown={(e) => e.stopPropagation()}
        >
