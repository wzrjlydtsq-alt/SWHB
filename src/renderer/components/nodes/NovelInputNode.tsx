0
import { FileText, Trash2 } from '../../utils/icons.tsx'
...
export const NovelInputNode = ({
  ...
  return (
    <div className="relative w-full h-full flex flex-col transition-colors pointer-events-auto bg-transparent">
      ...
      <div className="node-footer-controls flex items-center justify-between px-3 py-2 border-t border-[var(--border-color)] shrink-0 bg-[var(--bg-base)]">
        <button
          className="node-footer-primary flex-1 px-3 py-2 rounded text-xs font-mono font-bold transition-all bg-[var(--primary-color)]/10 text-[var(--primary-color)] hover:bg-[var(--primary-color)]/20 border border-[var(--primary-color)]/20 shadow-sm active:scale-95"
          onMouseDown={(e) => e.stopPropagation()}
          type="button"
          onClick={() => {
