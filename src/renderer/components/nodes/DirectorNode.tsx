...
import { DEFAULT_BASE_URL, getModelParams, calculateResolution } from '../../utils/constants.ts'
...
export const DirectorNode = React.memo(function DirectorNode({
  ...
  /* ── 持久化关键状态到 node.settings（防抖合并，避免高频更新导致级联重渲染） ── */
  ...
  const generateScript = useCallback(async () => {
    ...
    if (!modelId) {
      setScriptError('请先配置 Chat 模型')
      return
    }
    const config = cfgs.find((c) => c.id === modelId)
    if (!config) {
      setScriptError('未找到模型配置')
      return
    }
    const apiKey = config.key || globalApiKey
    if (!apiKey) {
      setScriptError('请先配置 API Key')
      return
    }
  ...
  const generateShotImage = useCallback(
    async (shotId) => {
      ...
      const shot = scriptData?.shots?.find((ss) => ss.id === shotId)
      if (!shot) {
        console.warn('[DirectorNode] shot not found:', shotId)
        return
      }
      const modelId = imageModelId || imageModels[0]?.id
      console.log(
        '[DirectorNode] modelId:',
        modelId,
        'imageModels:',
        imageModels.length,
        'cfgs:',
      ...
      const config = cfgs.find((c) => c.id === modelId)
      if (!config) {
        console.warn('[DirectorNode] config not found for modelId:', modelId)
        return
      }
      const apiKey = config.key || globalApiKey
      if (!apiKey) {
      ...
      const baseUrl = (config.url || DEFAULT_BASE_URL).replace(/\/+$/, '')
      const resolvedParams = (getModelParams(modelId, ratio, 'Auto') ||
        calculateResolution(ratio, 'Auto')) as any
      ...
      try {
        const response = await window.api.engineAPI.submitTask(
          withCurrentProjectTaskPayload(payload) as any
        )
  ...
  const generateAllShotImages = useCallback(() => {
    if (!scriptData?.shots?.length) {
      console.warn('[DirectorNode] no shots')
      return
    }
    ...
    const cleanup = window.api.engineAPI.onTaskUpdated((task) => {
      ...
      if (task.status === 'completed' && task.resultUrl) {
        setShotImages((prev) => ({
          ...
          [shotId]: {
            status: 'done',
            url: getXingheMediaSrc(task.resultUrl),
            taskId: tid,
            error: ''
          }
        }))
        ...
        setShotImages((prev) => ({
          ...
          [shotId]: {
            ...
            url: '',
            taskId: tid,
            error: friendlyError(task.error) || '生成失败'
          }
        }))
      }
    })
  ...
  const generateShotVideo = useCallback(
    async (shotId) => {
      ...
      const { sizeStr, w, h } = (getModelParams(modelId, ratio, 'Auto') ||
        calculateResolution(ratio, 'Auto')) as any
      setShotVideos((prev) => ({
        ...prev,
        [shotId]: { status: 'generating', url: '', taskId: '', error: '' }
      ...
      try {
        const response = await window.api.engineAPI.submitTask(withCurrentProjectTaskPayload({
  ...
  const imgDone = Object.values(shotImages).filter((v: any) => v.status === 'done').length
  const imgFailed = Object.values(shotImages).filter((v: any) => v.status === 'failed').length
  const vidDone = Object.values(shotVideos).filter((v: any) => v.status === 'done').length
  const vidFailed = Object.values(shotVideos).filter((v: any) => v.status === 'failed').length
  ...
  return (
    <div
      className="flex flex-col h-full text-[var(--text-primary)] pointer-events-auto nodrag nopan"
      style={{ minHeight: 0 }}
    >
      {/* ── 标题栏 ── */}
      <div
      ...
      <div
        className="text-[9px] text-[var(--text-muted)] px-4 py-1 border-b"
        style={{ borderColor: 'var(--border-color)' }}
      >
        {step === 1 && '描述想法，选择参数'}
        {step === 2 &&
          (scriptLoading
            ? '🎬 AI 正在创作剧本...'
            : `共 ${shotCount} 个分镜，总时长约 ${totalDuration}s`)}
        {step === 3 &&
          (hasImageStarted
      ...
      <div className="flex-1 overflow-y-auto min-h-0 custom-scrollbar">
        ...
        {step === 1 && (
          <div className="px-4 py-3 flex flex-col gap-3">
            <div>
              <label className="text-[10px] font-semibold text-[var(--text-secondary)] mb-1 block">
                💡 创意想法
              </label>
              <textarea
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1">
                {RATIOS.map((r) => (
                  <button key={r} onClick={() => setRatio(r)} className={selBtn(ratio === r)}>
                    {r}
                  </button>
                ))}
              </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1">
                {DURATIONS.map((d) => (
                  <button key={d} onClick={() => setDuration(d)} className={selBtn(duration === d)}>
                    {d}
                  </button>
                ))}
              </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1">
                {STYLES.map((ss) => (
                  <button key={ss} onClick={() => setStyle(ss)} className={selBtn(style === ss)}>
                    {ss}
                  </button>
                ))}
              </div>
              ...
              <div className="flex flex-wrap gap-1">
                {DIRECTIONS.map((d) => (
                  <button
                    ...
                    className={selBtn(direction === d.label)}
                  >
                    {d.label}
                  </button>
                ))}
            ...
            <div>
              <button
                ...
                className="flex items-center gap-1 text-[9px] text-[var(--text-muted)] hover:text-[var(--text-primary)]"
              >
                <ChevronRight
                  size={8}
                ...
                <div className="mt-1 flex flex-col gap-1.5 pl-2 border-l-2 border-[var(--border-color)]">
                  {[
                    ...
                    <div key={m.label} className="flex items-center gap-1.5">
                      <span className="text-[9px] text-[var(--text-muted)] w-8 shrink-0">
                        {m.label}
                      </span>
                      <select
                        className="flex-1 bg-[var(--bg-secondary)] text-[10px] text-[var(--text-primary)] p-1 rounded border border-[var(--border-color)] outline-none"
                        value={m.val}
        ...
        {step === 2 && (
          <div className="px-4 py-3 flex flex-col gap-3">
            ...
            {scriptError && (
              <div className="p-2 rounded-lg bg-red-500/10 border border-red-500/30 text-[10px] text-red-400">
                {scriptError}
              </div>
            )}
            {scriptData && !scriptLoading && (
              <>
                <div className="text-[12px] font-bold text-amber-400 mb-1">
                  📽 {scriptData.title}
                </div>
                <div className="flex flex-col gap-2">
                  {scriptData.shots?.map((shot) => (
                    <div
                      key={shot.id}
                      className="rounded-lg border border-[var(--border-color)] bg-[var(--bg-secondary)] p-2"
                    >
                      <details>
                        <summary className="cursor-pointer text-[10px] font-bold text-amber-300 flex items-center gap-1">
                          Shot {shot.id}{' '}
                          <span className="text-[var(--text-muted)] font-normal">
                            {shot.duration}s · {shot.prompt_cn?.slice(0, 30)}...
                          </span>
                        </summary>
                        <div className="mt-1.5 flex flex-col gap-1">
                          <div className="flex items-start gap-1">
                            <span className="text-[9px] text-[var(--text-muted)] w-8 shrink-0 mt-0.5">
                              EN
                            </span>
                            {editingShot?.id === shot.id && editingShot.field === 'prompt' ? (
                              <div className="flex-1 flex gap-1">
                                <textarea
                                  className="flex-1 bg-[var(--bg-base)] text-[9px] p-1 rounded border border-amber-500/50 outline-none resize-none h-12 text-[var(--text-primary)]"
                                  value={editValue}
                              ...
                              <p className="flex-1 text-[9px] text-[var(--text-secondary)] leading-relaxed">
                                {shot.prompt}
                                <button
                                  onClick={() => startEditShot(shot.id, 'prompt')}
                                  className="ml-1 text-[var(--text-muted)] hover:text-amber-400 inline"
                                >
                                  <Edit size={9} />
                                </button>
                              </p>
                          ...
                          <div className="flex items-start gap-1">
                            ...
                            {editingShot?.id === shot.id && editingShot.field === 'prompt_cn' ? (
                              <div className="flex-1 flex gap-1">
                                <textarea
                                  className="flex-1 bg-[var(--bg-base)] text-[9px] p-1 rounded border border-amber-500/50 outline-none resize-none h-10 text-[var(--text-primary)]"
                                  value={editValue}
                                  onChange={(e) => setEditValue(e.target.value)}
                                />
                                <button
          ...
          <div className="px-4 py-3 flex flex-col gap-2">
            {scriptData?.shots?.map((shot) => {
              ...
              return (
                <div
                  key={shot.id}
                  className={`rounded-lg border overflow-hidden ${isFailed ? 'border-red-500/50 bg-red-500/5' : 'border-[var(--border-color)] bg-[var(--bg-secondary)]'}`}
                >
                  <div className="flex gap-2 p-2">
                    <div
                      className={`w-24 h-16 rounded border flex items-center justify-center shrink-0 overflow-hidden ${isFailed ? 'border-red-500/30 bg-red-500/10' : 'border-[var(--border-color)] bg-[var(--bg-base)]'}`}
                    >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 mb-0.5">
                        ...
                        {isFailed && (
                          <span className="text-[8px] px-1 rounded bg-red-500/20 text-red-400 font-bold">
                            {img.error || '生成失败'}
                          </span>
                        )}
                      </div>
                      <p className="text-[8px] text-[var(--text-secondary)] line-clamp-2 leading-relaxed">
                        {shot.prompt_cn}
                      </p>
                      <button
                      ...
                      >
                        <RefreshCw size={8} />{' '}
                        {isFailed ? '⚡ 重试' : img.status === 'done' ? '重新生成' : '生成'}
                      </button>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="flex gap-2 p-2">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 mb-0.5">
                        ...
                        {vid.status === 'failed' && (
                          <span className="text-[8px] px-1 rounded bg-red-500/20 text-red-400">
                            {vid.error || '失败'}
                          </span>
                        )}
                      </div>
      ...
      {/* ── 底部操作栏 ── */}
      <div
        className="node-footer-controls flex items-center justify-between px-4 py-2 border-t shrink-0"
        style={{ borderColor: 'var(--border-color)' }}
      >
        {step === 1 && (
          <>
            ...
            <button
              ...
              disabled={!idea.trim()}
              className="node-footer-primary flex items-center gap-1 px-3 py-1.5 rounded-lg text-[11px] font-semibold transition-all hover:scale-105 disabled:opacity-40 disabled:hover:scale-100"
              style={{
        ...
        {step === 2 && !scriptLoading && (
          <>
            <button
              onClick={() => setStep(1)}
              className="flex items-center gap-0.5 px-2 py-1 rounded-lg text-[10px] text-[var(--text-secondary)] hover:bg-[var(--bg-hover)]"
            >
            ...
            <div className="flex items-center gap-1.5">
              <button
                onClick={generateScript}
                className="flex items-center gap-0.5 px-2 py-1 rounded-lg text-[10px] text-[var(--text-secondary)] hover:bg-[var(--bg-hover)] border border-[var(--border-color)]"
              >
                🔄 重新生成
              </button>
              {scriptData && (
                <button
                  onClick={() => setStep(3)}
                  className="node-footer-primary flex items-center gap-1 px-3 py-1.5 rounded-lg text-[10px] font-semibold hover:scale-105 transition-all"
        ...
        {step === 3 && (
          <>
            ...
            <div className="flex items-center gap-1.5">
              ...
              >
                <ImageIcon size={12} />{' '}
                {hasImageStarted ? '🔄 全部重新生成' : '🚀 开始生成全部分镜图'}
              </button>
              {imgFailed > 0 && (
                <button
                  onClick={() => {
          ...
          <>
            ...
            <div className="flex items-center gap-1.5">
              ...
              {Object.values(shotVideos).length > 0 &&
                Object.values(shotVideos).every((v: any) => v.status === 'done') && (
                  <>
                    <button
                      onClick={async () => {
                        setConcatExporting(true)
                        try {
                          const videoPaths = scriptData.shots
                            .map((ss) => shotVideos[ss.id]?.url)
                            .filter(Boolean)
                          const title = scriptData.title || 'ai_director'
                          const outputName = `${title.replace(/[^a-zA-Z0-9\u4e00-\u9fa5]/g, '_')}_${Date.now()}.mp4`
                          const result = await window.api.localCacheAPI.concatVideos(
                            videoPaths,
                            outputName,
                            getProjectCacheContext()
                          )
                          if (result.success) {
                            if ((result as any).exported !== false) alert(`✅ 已导出: ${result.path}`)
                          } else {
                            alert(`❌ ${result.error}`)
                          }
                        } catch (err) {
                      ...
                      className="node-footer-primary flex items-center gap-1 px-3 py-1.5 rounded-lg text-[10px] font-semibold hover:scale-105 transition-all disabled:opacity-60"
                      style={{
