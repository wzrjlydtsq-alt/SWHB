...
import { Trash2 } from '../../../utils/icons.tsx'
...
export function NodeShell({
  ...
  return (
    <div
      className={`relative h-full flex flex-col transition-colors pointer-events-auto bg-[var(--bg-panel)] rounded-xl overflow-hidden animate-in zoom-in-95 duration-200 ${className}`}
    ...
    >
      ...
      {/* 底部操作栏 */}
      {footer && (
        <div className="node-footer-controls px-3 py-2 border-t border-[var(--border-color)] shrink-0 flex items-center gap-2 bg-[var(--bg-panel)] rounded-b-xl">
          {footer}
        </div>
      )}
    </div>
