...
export const useCanvasDragLogic = ({
  ...
  const latestSelectionBoxRef = useRef(null)
  const panThrottleLastTsRef = useRef(0)

  latestSelectionBoxRef.current = selectionBox

  const handleMouseMove = useCallback(
    (e, canvasRef) => {
      const { clientX, clientY } = e
      const worldPos = screenToWorld(clientX, clientY)
      // 仅在连线时写�?store（拖�?平移时不需要触发全局更新�?
      ...
      if (isSelecting || isSelectingRef.current) {
        ...
        if (!selectionRafRef.current) {
          selectionRafRef.current = requestAnimationFrame(() => {
            ...
            const currentSelectionBox = latestSelectionBoxRef.current
            if (!currentSelectionBox) {
              selectionRafRef.current = null
              return
            }
            const nextSelectionBox = { ...currentSelectionBox, endX, endY }
            latestSelectionBoxRef.current = nextSelectionBox
            setSelectionBox(nextSelectionBox)

            const boxStartX = Math.min(nextSelectionBox.startX, endX)
            const boxStartY = Math.min(nextSelectionBox.startY, endY)
            const boxEndX = Math.max(nextSelectionBox.startX, endX)
            const boxEndY = Math.max(nextSelectionBox.startY, endY)
      ...
      if ((isPanning || isPanningRef.current) && !isSelectingRef.current) {
        ...
        if (nowTs - lastTs >= 32 && pendingPanUpdate.current) {
          ...
          setView((prev) => {
            const safeZoom = Math.max(CANVAS_MIN_ZOOM, Math.min(CANVAS_MAX_ZOOM, prev.zoom))
      ...
      if (resizingNodeId) {
        ...
        const safeZoom = Math.max(CANVAS_MIN_ZOOM, Math.min(CANVAS_MAX_ZOOM, view.zoom))
        ...
        // 组联动：如果拖动的节点属于某�?group，合�?group 内所有节�?
        ...
        const currentNodes = nodesRef.current
        const currentZoom = Math.max(CANVAS_MIN_ZOOM, Math.min(CANVAS_MAX_ZOOM, view.zoom))
        ...
        if (dragNodeIds.length > 1) {
          ...
          const updates = Array.from(
            (multiNodeDragStartPos.current.nodes as Map<string, { x: number; y: number }>).entries()
          )
        ...
        // 单节点拖动：完全�?ReactFlow �?onNodesChange 处理，不重复更新
      }
    },
    [
      isPanning,
