...
import { getVideoMetadata } from '../utils/fileHelpers.ts'
import { getDefaultDirectorState } from '../features/director/useDirectorStore.ts'
import { registerManagedBlobUrl, revokeManagedBlobUrl } from '../utils/blobUrlRegistry.ts'

// 获取当前项目 ID 的辅助函数
const getCurrentProjectId = () => useAppStore.getState().currentProject?.id || null
...
const customDebouncedSaveNode = (nodeData) => {
  ...
  const timer = setTimeout(() => {
    if (window.dbAPI?.nodes?.save) {
      ...
      // 确保有有效的projectId才保存节点
      if (projectId) {
        window.dbAPI.nodes.save(nodeData, projectId).catch(console.error)
      } else {
        console.warn('[customDebouncedSaveNode] 无法保存节点：缺少有效的项目ID', nodeData.id)
      }
    }
...
export const useNodesState = (apiConfigs = []) => {
          ...
          updates.forEach(({ nodeId, updater }) => {
            ...
            if (idx !== undefined) {
              ...
              if (updatedNode !== next[idx]) {
                ...
                hasChanges = true
                if (window.dbAPI?.nodes?.save) {
                  customDebouncedSaveNode(updatedNode)
                }
              }
            }
          })
  ...
  const deleteNode = useCallback((id) => {
    const node = useAppStore.getState().nodesMap.get(id)
    revokeManagedBlobUrl(node?.content)
  ...
  const handleVideoFileUpload = useCallback(
    async (nodeId: string, file: File) => {
      ...
      let content = registerManagedBlobUrl(URL.createObjectURL(file))

      let videoMeta = { duration: 0, w: 0, h: 0 }
      try {
        videoMeta = (await getVideoMetadata(content)) as any
      } catch (err) {
      ...
      setNodes((prev) =>
        prev.map((n) => {
          ...
          if (n.content !== content) revokeManagedBlobUrl(n.content)
          return {
            ...n,
            content,
            videoMeta,
  ...
  const addNode = useCallback(
    (
      ...
      const defaultSizes = {
        'gen-video': { w: 500, h: 380 },
        'gen-image': { w: 560, h: 300 },
