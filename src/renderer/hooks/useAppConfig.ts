...
export const useAppConfig = (
  ...
  const [apiConfigs, setApiConfigs] = useState(() => {
    ...
    // 自动补充新版本新增的默认模型（按 id 去重，不覆盖用户已有配置）
    const existingIds = new Set(configs.map((c) => c.id))
    for (const defaultConfig of DEFAULT_API_CONFIGS) {
      if (!existingIds.has(defaultConfig.id) && !DELETED_MODEL_IDS.includes(defaultConfig.id)) {
