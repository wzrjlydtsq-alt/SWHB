...
import { canReadFileAsDataUrl, getImageDimensions, extractKeyFrames } from '../utils/fileHelpers.ts'
import { splitGridImage } from '../services/midjourneyService.ts'
import { detectScenesAndCapture } from '../services/videoProcessingService.ts'
import { revokeManagedBlobUrl } from '../utils/blobUrlRegistry.ts'
import { withProjectCacheContext } from '../utils/projectCache.ts'

export const useMediaHandlers = ({ setNodes, nodesMap, selectedNodeIdsRef, screenToWorld }: any) => {
  const updateNodeContentWithUrl = async (nodeId, finalUrl, fileType, updateFn) => {
    let dimensions: any = { w: 0, h: 0 }
    ...
    updateFn((prev) =>
      prev.map((n) => {
        if (n.id === nodeId) {
          if (n.content !== finalUrl) revokeManagedBlobUrl(n.content)
  ...
  const handleFileUpload = useCallback(
    async (nodeId, e) => {
      ...
      try {
        ...
        if (!canReadFileAsDataUrl(file, file.type.startsWith('video') ? 'Video upload' : 'Image upload')) {
          return
        }
  ...
  const handleAudioFileUpload = useCallback(
    async (nodeId, file) => {
      ...
      try {
        if (file.path) {
          ...
          if (response?.success && response.url) {
            setNodes((prev) =>
              prev.map((n) => {
                if (n.id === nodeId) {
                  if (n.content !== response.url) revokeManagedBlobUrl(n.content)
                  return {
                    ...n,
        ...
        if (!canReadFileAsDataUrl(file, 'Audio upload')) {
          return
        }
        const reader = new FileReader()
        reader.onload = async (event) => {
  ...
  const handleSplitGridFromUrl = useCallback(
    async (imageUrl: string, options: Record<string, any> = {}) => {
      if (!imageUrl) return
      const {
        originX,
        originY,
      ...
      try {
        const croppedImages = (await splitGridImage(imageUrl)) as any[]
