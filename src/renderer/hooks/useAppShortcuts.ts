...
import { canReadFileAsDataUrl } from '../utils/fileHelpers'
    ...
    const handleKeyDown = (e) => {
      ...
      if (e.key === 'Backspace' || e.key === 'Delete') {
        ...
        if (currentSelectedIds && currentSelectedIds.size > 0) {
          ...
          const totalNodes = nodesRef.current?.length || 0
          if (
            totalNodes > 0 &&
            idsToDelete.length >= totalNodes &&
            !window.confirm(`确定删除画布上的全部 ${totalNodes} 个节点吗？`)
          ) {
            return
          }
